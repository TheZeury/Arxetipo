...
namespace arx
{
	struct CommandFormatter
	{
		...
		auto excute_expression(const CommandASTExpressionNode& expression) -> void {
			...
			//	excute_expression(*std::get<CommandASTFunctionCallNode>(expression.value).function_body);
			//	std::cout << '(';
			//	for (auto& argument : std::get<CommandASTFunctionCallNode>(expression.value).arguments) {
			//		excute_expression(argument);
			//		std::cout << ", ";
			//	}
			//	if (std::get<CommandASTFunctionCallNode>(expression.value).arguments.size()) {
			//		std::cout << "\b\b";
			//	}
			//	std::cout << ')';
			//	break;
			...
			//	std::cout << '(';
			//	excute_expression(*(std::get<CommandASTParenthesesNode>(expression.value).expression));
			//	std::cout << ')';
			//	break;
			//}
