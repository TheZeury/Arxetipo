...
void main() {
    ...
    vec4 ambientLight = light_color * ambient_value;
    diffuse_value = max(dot(light_direction, normalize(normal)), 0.0);
    vec4 diffuseLight = light_color * diffuse_value;
    out_color = (material_properties.enable_diffuse_map ? texture(diffuse_sampler, frag_uv) : material_properties.default_color) * (ambientLight + diffuseLight);
    if(out_color.w < 0.3)
    {
        discard;
    }
